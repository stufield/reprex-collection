...
#' * At each iteraton the training set is bootstrap sampled from class 1
#' *only*.
...
simres <- seq(190) %>%                 # more iterations -> no class 2 left; trouble fitting
  purrr::map_df(~ {
    ...
    class_prop <- train_boot %>%       # calc. proportion c1
      pull(y) %>% {. == 1} %>%         # pull out `y` column; how many are "1"?
...
#' Generate `ggplot` of class imbalance vs. prediction accuracy
#'
